...
/*   Updated: 2021/04/03 16:36:30 by amya             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_li(t_light *li, t_data_light *ligth, t_obj *obj, t_ray ray)
{
	...
	li->rm = sub_vect(vect_mult_val(obj->norm, 2 *
		vect_scal(li->rm, obj->norm)), li->rm);
	li->diff = vect_mult_val(obj->color, fmax(0.0, 0.5 *
		vect_scal(get_normalized(sub_vect(ligth->position, obj->hit)), obj->norm)));
	li->spec = vect_mult_val(ligth->color, 0.5 *
		pow(fmax(0.0, vect_scal(li->rm, get_normalized(
