...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/23 16:43:46 by amya              #+#    #+#             */
/*   Updated: 2021/04/23 16:43:50 by amya             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	load_texture(t_all *data, t_obj *sphere)
{
	data->id++;
	sphere->id = data->id;
	if (ft_strcmp(sphere->texture, ".") != 0)
	{
		sphere->surface = IMG_Load(sphere->texture);
		if (!sphere->surface)
